...
1. **Resolve the absolute artifact dir.** `PIPELINE_BASE="$(git rev-parse --show-toplevel)/.pipeline"`, `ARTIFACT_DIR="$PIPELINE_BASE/<issue>"`, passed fully expanded into the subagent prompt as `/pipeline` does. For a phase that runs inside the implementation worktree (`dev`, `qa`, `peer-review`), take `WORKTREE_PATH` and `ARTIFACT_DIR` from `node "${CLAUDE_PLUGIN_ROOT}/scripts/worktree.mjs" resolve --issue <issue> --seed-from "$PIPELINE_BASE/<issue>"` instead; exit 2 (none or AMBIGUOUS) halts and goes to the owner.
...
Re-runs the Phase 4 panel in parallel, writing per-agent shards that the orchestrator merges. Same prompt shape, panel-composition rule, and shard-merge step as `/pipeline` Phase 4. **Before you act, read the orchestrator files that hold those rules, with the Read tool:** `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-panel.md` (the panel-roles.mjs call and its exit codes, rendering, the shard merge), then `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-verdict.md` after the merge; on a delta re-run ALSO `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-delta.md` (the delta-set call) and `${CLAUDE_PLUGIN_ROOT}/orchestrator/loop-backs.md` (what a `checkpoint.mjs --loopback` exit 2 obliges). The summary below is not a substitute for them. This command mirrors `/pipeline` Phase 4 exactly, including the **delta re-review** semantics, so a manual re-run and the auto re-review never diverge.

- **First (full) run of a panel:** resolve and record the panel with `panel-roles.mjs full ... --write > "$ARTIFACT_DIR/roles-to-merge.txt"` exactly as `phase-4-panel.md` calls it (exit 0 or 21 dispatches, 21 meaning a seat came from an unevaluable probe and its `PANEL-NOTE` goes in the PR summary; 1 or anything else halts), dispatch all of them, and merge via `node "${CLAUDE_PLUGIN_ROOT}/scripts/merge-peer-review.mjs" --status <status.json> <peer-review.json> <role>=<shard> ...` against a freshly reset `peer-review.json`. The `--status` is what gives the merge the run's cost_class, which decides what may block.
- **Delta re-run (the panel already ran and Dev pushed fixes):** enter the round with `node "${CLAUDE_PLUGIN_ROOT}/scripts/checkpoint.mjs" enter 4-review --status <status.json> --commit`, which clears `final_verdict` and `peer_review_verdict_counts` (#110) and counts the round in the same write. Before Dev pushed those fixes, the fix round was entered with `checkpoint.mjs enter 3-impl --loopback`, which refuses (exit 2) past the budget and prints the owner decision block that `loop-backs.md` says how to handle; a manual re-run does not get around it. The original `panel_roles` in status.json is the authoritative FULL panel; do NOT shrink it. Resolve the delta set with `panel-roles.mjs delta ... --write > "$ARTIFACT_DIR/roles-to-merge.txt"` exactly as `phase-4-delta.md` calls it, with the same exit handling as the full run; it seeds the roles holding an open blocker id and adds a role only where the fix commits touched its merge-class surface. Render the delta with `--peer-review <peer-review.json>` so each lens lists its open blocker ids. A SecOps `VETO` that stands (valid `veto_ground`, blocking concern) halts to BA as always, through `checkpoint.mjs enter 1-ba --loopback`. Merge the delta shards INTO the existing `peer-review.json` with the same `merge-peer-review.mjs` invocation (do NOT reset the file), so the standing verdicts of the NON-delta roles are preserved; the merge normalizes every shard it folds under the materiality rule (`scripts/materiality.mjs`), so the rubric reads the recorded `verdict` and the reviewer's own is kept as `verdict_as_returned` when the two differ. Record the verdict with `record-verdict.mjs --note "delta re-review: <the roles in roles-to-merge.txt>"` exactly as `phase-4-verdict.md` does; it counts over the FULL `panel_roles` and lists every panel role in the PR summary.

**Do not re-spell the roster.** Both rosters come from `${CLAUDE_PLUGIN_ROOT}/scripts/panel-roles.mjs`; never compose one by hand or with a fresh `node -e`, and never pipe the call, because the exit status is the contract.
