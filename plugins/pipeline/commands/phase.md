...
- **Delta re-run (the panel already ran and Dev pushed fixes):** in the checkpoint write that sets `current_phase: "4-review"` for this round, clear `final_verdict` and `peer_review_verdict_counts` to `null` (#110). A run under re-review has not concluded, and leaving the prior round's verdict attached makes every control that reads `final_verdict` as a conclusion blind for the whole remediation window; `phase-4-delta.md` carries the same rule and the evidence for it. Before Dev pushed those fixes, the fix round was counted: `node "${CLAUDE_PLUGIN_ROOT}/scripts/round-budget.mjs" enter fix-round --status <status.json>` exits 0 inside the budget (tooling 1, product 2, product-money 2) and exits 2 past it, printing the owner decision block (ship with deferrals, split, stop), which `loop-backs.md` says how to handle; a manual re-run does not get around it. The original `panel_roles` in status.json is the authoritative FULL panel; do NOT shrink it. Seed the delta set with the roles that hold an OPEN BLOCKER id (`materiality.open_blocker_ids` in `peer-review.json`, via `openBlockerRoles` in `scripts/materiality.mjs`), THEN add a role only where the fix commits touched its MERGE-CLASS surface, through the same probes as `phase-4-delta.md`: `dba` on a data-layer path (`diffTouchesDataLayer`), `secops` on a security-surface path (`diffTouchesSecuritySurface`) and on a data-layer path except at cost_class tooling, `qa` on a test file (`diffTouchesTests`), both of those from `scripts/security-surface.mjs`, and `devops` on infra (`diffTouchesInfra`) except at cost_class tooling. `design_review` has no merge-class surface and re-sits only while it holds an open blocker. No role is seated unconditionally on a delta round; SecOps seating on a FULL round stays mandatory. Render the delta with `--peer-review <peer-review.json>` so each lens lists its open blocker ids. A SecOps `VETO` that stands (valid `veto_ground`, blocking concern) halts to BA as always, after `round-budget.mjs enter spec-revision`. Merge the delta shards INTO the existing `peer-review.json` with the same `merge-peer-review.mjs` invocation (do NOT reset the file), so the standing verdicts of the NON-delta roles are preserved; the merge normalizes every shard it folds under the materiality rule (`scripts/materiality.mjs`), so the rubric reads the recorded `verdict` and the reviewer's own is kept as `verdict_as_returned` when the two differ. Compute `peer_review_verdict_counts` and the final verdict over the FULL `panel_roles`, list every panel role in the PR summary, and record the re-dispatched subset in the phase event note (`panel_roles` stays the original full panel for audit).
