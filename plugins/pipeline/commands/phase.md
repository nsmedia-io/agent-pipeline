...
- **Delta re-run (the panel already ran and Dev pushed fixes):** in the checkpoint write that sets `current_phase: "4-review"` for this round, clear `final_verdict` and `peer_review_verdict_counts` to `null` (#110). A run under re-review has not concluded, and leaving the prior round's verdict attached makes every control that reads `final_verdict` as a conclusion blind for the whole remediation window; `/pipeline`'s Phase 4 checkpoint carries the same rule and the evidence for it. The original `panel_roles` in status.json is the authoritative FULL panel; do NOT shrink it. Seed the delta set with the objecting role(s), THEN add any role whose SURFACE the fix commits touched, through the same probes as `/pipeline`: `dba` and `secops` on a data-layer path, `devops` on infra, `design_review` on a frontend path, `secops` on a security-surface path (`diffTouchesSecuritySurface`) and `qa` on a test file (`diffTouchesTests`), both from `scripts/security-surface.mjs`. No role is seated unconditionally; a standing round-1 verdict holds unless its owner objected or its surface moved. A SecOps `VETO` on a named `veto_ground` halts to BA as always. Merge the delta shards INTO the existing `peer-review.json` with the same `merge-peer-review.mjs` invocation (do NOT reset the file), so the standing verdicts of the NON-delta roles are preserved; the merge normalizes every shard it folds under the materiality rule (`scripts/materiality.mjs`), so the rubric reads the recorded `verdict` and the reviewer's own is kept as `verdict_as_returned` when the two differ. Compute `peer_review_verdict_counts` and the final verdict over the FULL `panel_roles`, list every panel role in the PR summary, and record the re-dispatched subset in the phase event note (`panel_roles` stays the original full panel for audit).

**The surface probe: run the shared definition, do not re-spell it.** The delta round's surface test is the `surface_probe` shell function defined in the delta-re-review block of `${CLAUDE_PLUGIN_ROOT}/commands/pipeline.md`. That block is the ONE definition; copy it into your shell from there rather than writing a fresh `node -e`, and probe `diffTouchesDataLayer` for DBA, `diffTouchesInfra` for DevOps, and `diffTouchesFrontend` for Design.
