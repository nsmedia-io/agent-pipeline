...
- **Delta re-run (the panel already ran and Dev pushed fixes):** in the checkpoint write that sets `current_phase: "4-review"` for this round, clear `final_verdict` and `peer_review_verdict_counts` to `null` (#110). A run under re-review has not concluded, and leaving the prior round's verdict attached makes every control that reads `final_verdict` as a conclusion blind for the whole remediation window; `/pipeline`'s Phase 4 checkpoint carries the same rule and the evidence for it. The original `panel_roles` in status.json is the authoritative FULL panel; do NOT shrink it. Seed the delta set with `DELTA="qa secops"` UNCONDITIONALLY (both re-review the fix commits on every delta round; SecOps is never-trimmed, so its round-1 APPROVE never stands in on a delta round, exactly like QA's), THEN add the objecting role(s), THEN add any role whose SURFACE the fix commits touched. SecOps's VETO semantics are unchanged. Merge the delta shards INTO the existing `peer-review.json` with the same `merge-peer-review.mjs` invocation (do NOT reset the file), so the standing approvals of the NON-delta roles are preserved while QA and SecOps are always freshly re-reviewed. Compute `peer_review_verdict_counts` and the final verdict over the FULL `panel_roles`, list every panel role in the PR summary, and record the re-dispatched subset in the phase event note (`panel_roles` stays the original full panel for audit).
