...
- **Trivial/standard tier:** Dev authors code and tests together. If `<ARTIFACT_DIR>/constraints.md` is missing, read `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-2-lite.md` now and extract it first exactly as that file does (the `sed` over the marker-delimited "Standard-tier constraints" blocks in the three specialist agent files under `${CLAUDE_PLUGIN_ROOT}/agents/{dba,devops,secops}.md`), then prompt:
...
- **Phase 4 (binding adversarial verdict):** independent coverage audit on the finished diff (remote CI runs concurrently; CI-green is verified at merge, not required to enter the panel). This is QA's binding test verdict. It is an ADVERSARIAL gap-check, not an auto-pass on green. Prompt:
  - "Audit test coverage against the current diff, impl-report.json, spec.json, and your own Phase-3 behavioral tests with fresh eyes. Green proves only that existing tests pass; hunt for the tests that should exist and do not. Verify every changed path is tested, webhooks cover idempotency/replay, integration tests hit a real data layer, failure modes are covered, and behavior outside your Phase-3 tests is not left untested (overfitting). Return a binding verdict APPROVE | APPROVE_WITH_NOTES | REQUEST_CHANGES | REQUEST_REFACTOR. If you find a gap, name the missing test; do not write it yourself (that loops back to Dev). A gap blocks only when it lets a gate or test report green on a real failure (merge_class wrong-pass) or hides a money, data-loss or security-exposure defect; otherwise it is a note. Record `test_cost` in your shard: assertion count and pre-push seconds before and after the change."

### peer-review

Re-runs the Phase 4 panel in parallel, writing per-agent shards that the orchestrator merges. Same prompt shape, panel-composition rule, and shard-merge step as `/pipeline` Phase 4. **Before you act, read the orchestrator files that hold those rules, with the Read tool:** `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-panel.md` (panel composition, the tooling block, rendering, the shard merge, and the three-outcome probe contract), then `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-verdict.md` after the merge; on a delta re-run ALSO `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-delta.md` (the delta set, the surface probe) and `${CLAUDE_PLUGIN_ROOT}/orchestrator/loop-backs.md` (what a `checkpoint.mjs --loopback` exit 2 obliges). The summary below is not a substitute for them. This command mirrors `/pipeline` Phase 4 exactly, including the **delta re-review** semantics, so a manual re-run and the auto re-review never diverge.

- **First (full) run of a panel:** resolve `PANEL_ROLES` by tier (all six at architectural; `qa secops` at trivial; `ba dev qa secops` plus surface-conditional `dba`/`devops` at standard; plus the surface-conditional `design_review` lens at EVERY tier when the diff touches a frontend surface; and at status.json `cost_class: "tooling"`, whatever the tier, `qa secops` plus ONE surface specialist, per the tooling block in `phase-4-panel.md`), record it in status.json `panel_roles`, dispatch all of them, and merge via `node "${CLAUDE_PLUGIN_ROOT}/scripts/merge-peer-review.mjs" --status <status.json> <peer-review.json> <role>=<shard> ...` against a freshly reset `peer-review.json`. The `--status` is what gives the merge the run's cost_class, which decides what may block.
- **Delta re-run (the panel already ran and Dev pushed fixes):** enter the round with `node "${CLAUDE_PLUGIN_ROOT}/scripts/checkpoint.mjs" enter 4-review --status <status.json> --commit`, which clears `final_verdict` and `peer_review_verdict_counts` (#110) and counts the round in the same write. Before Dev pushed those fixes, the fix round was entered with `checkpoint.mjs enter 3-impl --loopback`, which refuses (exit 2) past the budget and prints the owner decision block that `loop-backs.md` says how to handle; a manual re-run does not get around it. The original `panel_roles` in status.json is the authoritative FULL panel; do NOT shrink it. Seed the delta set with the roles that hold an OPEN BLOCKER id (`materiality.open_blocker_ids` in `peer-review.json`, via `openBlockerRoles` in `scripts/materiality.mjs`), THEN add a role only where the fix commits touched its MERGE-CLASS surface, through the same probes as `phase-4-delta.md`: `dba` on a data-layer path (`diffTouchesDataLayer`), `secops` on a security-surface path (`diffTouchesSecuritySurface`) and on a data-layer path except at cost_class tooling, `qa` on a test file (`diffTouchesTests`), both of those from `scripts/security-surface.mjs`, and `devops` on infra (`diffTouchesInfra`) except at cost_class tooling. `design_review` has no merge-class surface and re-sits only while it holds an open blocker. No role is seated unconditionally on a delta round; SecOps seating on a FULL round stays mandatory. Render the delta with `--peer-review <peer-review.json>` so each lens lists its open blocker ids. A SecOps `VETO` that stands (valid `veto_ground`, blocking concern) halts to BA as always, through `checkpoint.mjs enter 1-ba --loopback`. Merge the delta shards INTO the existing `peer-review.json` with the same `merge-peer-review.mjs` invocation (do NOT reset the file), so the standing verdicts of the NON-delta roles are preserved; the merge normalizes every shard it folds under the materiality rule (`scripts/materiality.mjs`), so the rubric reads the recorded `verdict` and the reviewer's own is kept as `verdict_as_returned` when the two differ. Record the verdict with `record-verdict.mjs --note "delta re-review: <subset>"` exactly as `phase-4-verdict.md` does; it counts over the FULL `panel_roles` and lists every panel role in the PR summary.

**The surface probe: run the shared definition, do not re-spell it.** The delta round's surface test is the `surface_probe` shell function defined in the delta-re-review block of `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-delta.md`. That block is the ONE definition; copy it into your shell from there rather than writing a fresh `node -e`, and probe `diffTouchesDataLayer` for DBA, `diffTouchesInfra` for DevOps, and, on a FULL run only, `diffTouchesFrontend` for Design.

**It has three outcomes, never two, and the third one SEATS.** `surface_probe` exits 0 on a MATCH, **20** on a NO-MATCH, and ANY other exit means INDETERMINATE: the module was absent, an export was renamed, the path list could not be read, or `node` itself was missing. Add the role unless the exit is exactly 20. When the exit is neither 0 nor 20, add the role anyway and print `PANEL-NOTE: <role> SEATED on an INDETERMINATE <surface> probe (exit $RC), not on a match.`, then record that sentence in `status.json` (`flags`) — otherwise an auditor reading `panel_roles` cannot tell a seat earned by a match from one earned by a broken plugin root. Never write the probe as `surface_probe ... | grep -q ...`: a pipe discards the exit status, which is the entire mechanism. The reasoning behind the reserved code, and why an unevaluable check must not read as a clean diff, is stated once in `${CLAUDE_PLUGIN_ROOT}/orchestrator/phase-4-panel.md` under "Three outcomes, never two, and the third one SEATS"; this command inherits that contract rather than keeping a second copy of it.
...
2. Append the `<phase-token>-rerun` event: `node "${CLAUDE_PLUGIN_ROOT}/scripts/checkpoint.mjs" rerun <phase name> --status "$PIPELINE_BASE/<issue>/status.json" --verdict <verdict>` (add `--panel` for a reviewer re-run at Phase 4). The script owns the role-to-token mapping. The token prefix is load-bearing rather than cosmetic: a phase-less label resolves to no phase at all, so a re-run that did real work would leave a record that cannot clear the phase-entry guard, and the owner would be refused at their own turn boundary for work they just watched complete.
