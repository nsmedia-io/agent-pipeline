...
Artifact directory (absolute): <ARTIFACT_DIR>. Read and write artifacts only at this absolute path; never resolve .pipeline from cwd. If your sandbox refuses a write there, write the shard beside your own worktree at <WORKTREE_PATH>/.pipeline/<issue>/ and say so in your reply, naming the full path; do not silently drop it.
