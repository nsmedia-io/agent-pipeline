...
The property-not-the-fix rule binds every role at every tier as a contract, but its MACHINE-REQUIRED half reaches only the architectural tier's Phase 2 fan-out, where a reviewer writes `concerns[]` or `vulnerabilities[]` into a `review.<role>.json` shard; the standard tier's `constraints.md` injection writes no reviewer artifact and is the named exception the requirement does not touch.
