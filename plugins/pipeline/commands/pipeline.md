...
`--surface workflow` exists for the Workflow tool's `agent(prompt, { effort })`, which IS a genuine per-call override. **No phase dispatches through it yet**, and proposing one is blocked on #101 q4/q6 (whether a Workflow-dispatched SecOps `VETO` stays fail-CLOSED, and whether the runtime honours a `SubagentStop` block for a Workflow-dispatched subagent). On that surface the resolver ALWAYS emits an explicit token, including for a role with no table row, because omitting is not a safe default here: the Workflow docs say an omitted `effort` inherits the SESSION effort while #98's live spike observed it inheriting the agent's own frontmatter, and a session sitting at `low` makes those two readings differ in the direction that matters.
