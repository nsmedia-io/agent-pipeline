...
6. **Triage severity and set the risk tier. This call is load-bearing; when in doubt, promote.** Set `risk_tier` in the spec to one of `trivial | standard | architectural`; the orchestrator scales pipeline SHAPE by it. At the standard tier there is no pre-code specialist review (the orchestrator injects standing constraint checklists into the single Dev thread instead, and the specialists see only the finished diff at Phase 4), so the tier decision is what routes an ask around the deep gates. Criteria:
   ...
   **The `pipeline.config.json` literal above, the `compliance` domain, and the concrete triggers are a FLOOR, not the whole rule.** After writing the spec, run `node "${CLAUDE_PLUGIN_ROOT}/scripts/tier-floor.mjs" --spec "<ARTIFACT_DIR>/spec.json"` from the checkout under review. It unions `architecturalTriggers` from that checkout's `pipeline.config.json` onto the floor (config can only add a trigger), and any exit other than 0 halts: 2 means the spec is under-tiered (set `architectural`), and anything else means the floor was not computed (fix the spec and re-run; never read it as a pass). Its `ADVISORY:` keyword lines are hints for your judgement, never a trigger, and only a trigger it prints as a `REASON:` may be credited to the config.
