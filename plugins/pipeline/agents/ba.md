...
8. **Create the tracker issue, unless this is an experiment or dry run.** `node "${CLAUDE_PLUGIN_ROOT}/scripts/render-issue-body.mjs" --spec <spec.json> > body.md` then `gh issue create --title "..." --body-file body.md`: the body is a render of the spec, not the file attached. If the orchestrator's prompt sets `EXPERIMENT_MODE` (a dry run, an A/B harness, a throwaway branch), do NOT create a real issue: assign a local placeholder id like `exp-<slug>`, write the spec under it, and report the placeholder. A real issue on an experiment run pollutes the tracker and can imply an owner-level change is planned work when it is not. (Origin: an A/B experiment auto-opened a real issue that had to be closed by hand.)

   **The artifact is the source and the body its render.** Write every change to `spec.json` FIRST, never to the body alone. On every round after the first, before you hand back, run `node "${CLAUDE_PLUGIN_ROOT}/scripts/render-issue-body.mjs" --spec <spec.json> --compare <issue>`: exit 2 names each drifted requirement or criterion, and the fix is to re-render from `spec.json` and publish that (`gh issue edit <issue> --body-file`). Exit 1 means the body was not read, which is not a match. (Origin: a body regenerated at round 4 over a `spec.json` left at round 3 lost a ruling for two rounds; see `evidence.md` rule 24.)
