...
1. **Work in the issue worktree, never the root checkout.** The orchestrator passes `WORKTREE_PATH`; `cd` there. Standalone (`/phase dev`) with none given, run `node "${CLAUDE_PLUGIN_ROOT}/scripts/worktree.mjs" create --issue <issue> --type <fix|feat|chore> --slug <short-desc>`: it reuses the issue's existing worktree or creates one on `<type>/<issue>-<short-desc>` from the integration branch, and prints the absolute `WORKTREE_PATH=` and `ARTIFACT_DIR=`. Exit 2 (AMBIGUOUS, or the branch is checked out in the root checkout) means stop and ask the orchestrator. Record the absolute worktree path as `worktree_path` in `<ARTIFACT_DIR>/tasks.json` so QA can land in the same place. Fail-fast: do NOT implement, commit, or write artifacts from the root checkout.
...
8. **Emit `requirement_checks` before declaring done.** Start from `node "${CLAUDE_PLUGIN_ROOT}/scripts/scaffold-requirement-checks.mjs" --spec <ARTIFACT_DIR>/spec.json`: it prints `requirement_checks` (one entry per `spec.requirements` item, index and first 80 characters) and `acceptance_criteria_met` (one entry per criterion), each with `ac_id` set by the same own-label rule `gate-pre-phase4.mjs` covers criteria by. Fill every `status` (`PASS`, `PARTIAL`, `SKIP`), `notes`, `met` and `evidence`, add a `justification` to any `PARTIAL` or `SKIP`, and copy both arrays into the report. Do not drop an entry: a criterion named in neither array halts the gate, and a conditional or not-applicable one still gets its entry with the reason ("N/A, no migration shipped").
