...
5. **Implement incrementally against the behavioral test contract.** One logical unit per commit. Conventional commit messages (`fix:`, `feat:`, `chore:`, etc.) with issue reference.
   ...
   - **Config-tier backstop.** A diff that touches `pipeline.config.json` itself under a non-architectural tier is a mis-tier: STOP and report it to the orchestrator, per the tripwire above. No script evaluates a diff against this trigger; you are the check for it. See #76 for the mechanical seat this awaits.
...
8. **Emit `requirement_checks` before declaring done.** Before writing the implementation report, walk `spec.requirements` and emit one entry per item in a `requirement_checks` array. Each entry:
   ...
   Coverage is judged against `spec.acceptance_criteria`, not only `spec.requirements`, and it may be discharged in EITHER of two places: a `requirement_checks` entry, or an `acceptance_criteria_met` entry. The fail-closed pre-Phase-4 gate (`${CLAUDE_PLUGIN_ROOT}/scripts/gate-pre-phase4.mjs`) reads both arrays as one population and marks a criterion covered when some entry shares its ACn label OR overlaps a majority of its tokens. **What is NOT optional is that every AC label appears in one of them.** Put the criteria wherever the artifact reads best (`requirement_checks` is contracted as one entry per `spec.requirements`, so criteria that do not map one-to-one onto requirements belong in `acceptance_criteria_met`); a conditional or not-applicable criterion still gets an entry, with the reason written down, such as "N/A, no migration shipped". A criterion named in neither array halts the pipeline at the gate.
...
11. **Run both pre-Phase-4 gates yourself, before you return.** Against your own artifacts, from your worktree, and PASTE THEIR OUTPUT in your reply:
    ```bash
    node "${CLAUDE_PLUGIN_ROOT}/scripts/gate-pre-phase4.mjs" --issue <issue> \
      --impl-report "<ARTIFACT_DIR>/impl-report.json" --spec "<ARTIFACT_DIR>/spec.json"
    node "${CLAUDE_PLUGIN_ROOT}/scripts/gate-pre-phase4-frontend.mjs" --issue <issue> \
      --impl-report "<ARTIFACT_DIR>/impl-report.json"
    ```
    A gate that refuses is YOURS to fix before you hand off. The orchestrator runs both again at the Phase 3 to 4 transition, and a refusal discovered there costs a full round trip through the most expensive transition in the pipeline for something you could have read in one command. Both gates are cheap, deterministic and read-only. `SKIP` from the frontend gate is a pass.
