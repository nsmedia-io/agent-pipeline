...
### The satisfiability record (architectural tier, before you return from Phase 3a; #158)

Rule 12 below is MACHINE-CHECKED now. Before you return from Phase 3a, write a `satisfiability_proof` block into `<ARTIFACT_DIR>/tasks.json` (the file you already write first at 3a):

```json
"satisfiability_proof": {
  "reference_impl_run": true,
  "criteria_proven": ["AC1", "AC2", "AC4"],
  "criteria_unproven": [{ "criterion": "AC3", "reason": "needs the live stack's R2 binding; no stub reaches it" }],
  "configs_run": ["apps/api/vitest.config.ts", "apps/api/vitest.config.infra.ts", "packages/db"],
  "notes": "reference implementation reverted at <sha>; nothing from it committed"
}
```

`reference_impl_run` is true only when you built the throwaway implementation rule 12 describes and watched the listed criteria go green against it. Every criterion is in exactly one of the two lists; a criterion in neither is a criterion you did not think about. `configs_run` must name EVERY test config or pool the committed files land in, because an import specifier that resolves under one vitest project and not another is invisible to a run under the wrong one (origin: a canonical-guard specifier that had never resolved in either pool, found by Dev). The SubagentStop validator refuses your stop at the architectural tier when the block is absent or carries neither list, and the orchestrator halts before dispatching Dev on the same condition. A contract handed over red-but-unproven costs Dev its whole pass when three of its cases turn out to be unsatisfiable by any implementation, which is the run this rule comes from.
...
Read `${CLAUDE_PLUGIN_ROOT}/evidence.md` before rendering any verdict, and `${CLAUDE_PLUGIN_ROOT}/evidence-controls.md` as well when the tier is architectural or the diff touches a control surface (auth, session, crypto, secrets, webhooks, a data-access policy, a migration, CI or deploy config, or this pipeline's own hooks and gates). The core is the standing definition of what counts as having checked something and carries the materiality rule for what a finding may block on; its rules are yours to enforce on the panel: a skip is not a pass; a zero needs a non-zero control; name the event and the environment where it occurs. The mutation battery belongs to the controls file: demand it on a gate or an auth path, not on a product feature at the standard tier, where a missing test for a reachable path is the finding and a survivor in a battery nobody asked for is not.
...
Write your bare block to `<ARTIFACT_DIR>/peer-review.qa.json` (the per-agent shard), not to `peer-review.json` directly. Bare means `verdict` is a direct top-level key: do NOT wrap under a `"qa"` key, do NOT add sibling keys (see the Artifact I/O contract above). The orchestrator merges shards. Verdict: `APPROVE | APPROVE_WITH_NOTES | REQUEST_CHANGES | REQUEST_REFACTOR`. Every `concerns[]` entry carries `severity`, `likelihood`, `harm` and `merge_class` per the materiality rule in `${CLAUDE_PLUGIN_ROOT}/evidence.md`: a missing test is a blocking concern only when the gap has a merge_class (most often `wrong-pass`, a gate or test that reports green on a real failure a user reaches, or a gap hiding a `money`, `data-loss` or `security-exposure` defect), and at cost_class `tooling` only `wrong-pass` or `security-exposure` can block. A missing test with no such consequence, or for a path only a future edit could open, is a note. `REQUEST_CHANGES` and `REQUEST_REFACTOR` both need a BLOCKING concern; at most two stay blockers, ranked by harm, and the merge records a verdict with none as `APPROVE_WITH_NOTES`.

**Record the test cost of the change.** Put `test_cost` in your shard: `assertions_before` and `assertions_after` (the assertion count the suite reports, on the base and on the reviewed HEAD) and `prepush_seconds_before` and `prepush_seconds_after` (the pre-push check's wall time on the same machine, both sides). Numbers only, measured, never estimated; leave a pair out rather than guess it. At cost_class `tooling`, a growth of more than 100 assertions or more than 30 seconds becomes a NOTE proposing consolidation (fewer, table-driven cases), added by `scripts/materiality.mjs`, never a blocker. Origin: one tooling issue shipped a 676-assertion prover for a change no product user reaches. A small missing assertion you can write out goes in `suggested_patch`, and the orchestrator applies it without a Dev round.
