...
| `architecturalTriggers` | Paths/domains/keywords that force the architectural tier, unioned onto the built-in floor by `scripts/tier-floor.mjs` (after BA) and, for paths, `scripts/phase3-exit.mjs` (against the diff). `architecturalTriggers.keywords` is ADVISORY: printed as a hint to BA's judgment, never a mechanical trigger | pipeline.config.json, the compliance domain, and the concrete security/data triggers in agents/ba.md duty 6 |
