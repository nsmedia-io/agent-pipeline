...
Twelve changes in this release alter BEHAVIOUR on an existing project at its next plugin update, with no action on your part. There is no migration to run; there are twelve things to know.
...
11. **Absolute filesystem paths are redacted when a run is archived into the knowledge store.** `archiveIssue` now rewrites by value SHAPE across the whole document, keys included: paths under the repo root become repo-relative, everything else becomes `<redacted-absolute-path>`. This exists because `tasks.json` and `impl-report.json` both carry `worktree_path` absolutely and the archive is committed, so a developer's home directory was reaching a public tree. Note the current predicate replaces the WHOLE value of any string beginning with a slash, so a descriptive string like `/tmp/x (detached at abc123)` loses its prose as well as its path. That over-reach is tracked and is the safe direction, not the intended one.
