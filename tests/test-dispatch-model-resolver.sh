...
for cand in $(grep -oE 'scripts/[a-zA-Z0-9_-]+\.mjs' "$PIPELINE_MD" | sort -u); do
  ...
  case "$base" in
    ...
    # #164: the owner-facing moment, the issue-body render and the coverage skeleton. None takes a
    # (role, tier, phase) triple or resolves a model. Excluded by NAME, like the rest.
    ...
    # #164: the Phase 3 worktree lookup, the Phase 4 isolation check and Phase 0 setup. None resolves a
    # model or takes a (role, tier, phase) triple. Excluded by NAME, like the rest.
