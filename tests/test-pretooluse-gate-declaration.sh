...
# 0.42.0's render-panel.mjs, 24 with the review-convergence round-budget.mjs, 26 with version-check.mjs and migrate-records.mjs, 27 with C2's toon.mjs. It went red on schedule when the eighteenth
...
assert_eq "AC36: scripts/ holds 27 modules -- R6's LEAF module plus #117's check-status-record.mjs, not the reviewed commit's 16" \
  "$MODULE_N" "27"
