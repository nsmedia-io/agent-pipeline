...
# 0.42.0's render-panel.mjs, 25 with version-check.mjs and migrate-records.mjs. It went red on schedule when the eighteenth
...
assert_eq "AC36: scripts/ holds 23 modules -- R6's LEAF module plus #117's check-status-record.mjs, not the reviewed commit's 16" \
  "$MODULE_N" "25"
